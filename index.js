...
async function handleRequest(request) {
  ...
  if (request.method === "POST") {
    ...
    let req_cmd=req["cmd"]
    if (req_cmd == "add") {
      //let req_url=atob(req["url"])
      var req_url;
      try {
        req_url = atob(req["url"]);
      } catch (error) {
        req_url = req["url"];
      }
      ...
      if (config.unique_link){
        const url= await LINKS.get(req_keyPhrase);
        let url_sha512 = await sha512(url);        
        console.log(req_keyPhrase);
        console.log(url);
        console.log(url_sha512);
        await LINKS.delete(url_sha512);
  ...
  console.log(path)
  if(!path){
    return Response.redirect("https://psub.gilbert0571.eu.org/", 302)
